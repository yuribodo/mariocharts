...
it.each([3, 4, 5, 6, 12])(
  "keeps wrapping boxes for %p axes inside narrow and wide frames",
  (count) => {
    for (const width of [180, 300, 600]) {
      const height = 336;
      const layout = getRadarLayout(width, height, true, 30);
      for (let i = 0; i < count; i++) {
        const box = getAxisLabelBox(
          calculateAxisAngle(i, count),
          layout,
          width,
        );
