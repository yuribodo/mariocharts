...
const DEFAULT_COLORS = ['#3b82f6', '#10b981', '#f59e0b', '#ef4444', '#8b5cf6', '#06b6d4'] as const;
const DEFAULT_HEIGHT = 300;
...
const states = {
  Loading: ({ height = DEFAULT_HEIGHT }: { height?: number }) => (
    <div className="relative w-full flex items-center justify-center" style={{ height }}>
      <div className="w-full max-w-full p-6">
        <div className="animate-pulse bg-muted rounded h-4 w-32 mb-4" />
        <div className="relative border-l border-b border-muted/30" style={{ height: height - 100, margin: '0 50px 40px 50px' }}>
          <svg width="100%" height="100%" className="absolute inset-0">
            {Array.from({ length: 12 }).map((_, i) => (
              <circle key={i} cx={\`\${15 + (i % 4) * 25}%\`} cy={\`\${20 + Math.floor(i / 4) * 30}%\`} r={6 + (i % 3) * 4} className="fill-muted animate-pulse" style={{ animationDelay: \`\${i * 0.08}s\` }} />
            ))}
          </svg>
        </div>
      </div>
...
function ScatterPlotComponent<T extends ChartDataItem>({
  data, x, y, colors = DEFAULT_COLORS, className, height = DEFAULT_HEIGHT, loading = false, error = null, animation = true,
  series, size, sizeRange = DEFAULT_SIZE_RANGE, showTrendLine = false, trendLineColor, showLegend = false, showGrid = false, gridStyle = 'dashed', xDomain, yDomain, onPointClick,
}: ScatterPlotProps<T>) {
  const [containerRef, containerWidth] = useContainerDimensions();
  ...
  const { processedSeries, xTicks, yTicks, effectiveXDomain, effectiveYDomain } = useMemo(() => {
    if (!data.length || chartWidth <= 0 || chartHeight <= 0) return { processedSeries: [], xTicks: [], yTicks: [], effectiveXDomain: [0, 1] as const, effectiveYDomain: [0, 1] as const };
    const rawPoints = data.map((item, index) => {
      ...
      const sizeVal = typeof size === 'number' ? size : size ? getNumericValue(item, size as string) ?? DEFAULT_POINT_SIZE : DEFAULT_POINT_SIZE;
      const seriesKey = series ? String(item[series]) : 'default';
      return { data: item, index, xValue: xVal, yValue: yVal, sizeValue: sizeVal, seriesKey, label: series ? String(item[series]) : \`Point \${index + 1}\` };
    }).filter(p => p.xValue !== null && p.yValue !== null);
    if (!rawPoints.length) return { processedSeries: [], xTicks: [], yTicks: [], effectiveXDomain: [0, 1] as const, effectiveYDomain: [0, 1] as const };
    ...
    const sizeMin = Math.min(...sizeValues), sizeMax = Math.max(...sizeValues);
    ...
    rawPoints.forEach(point => { const existing = seriesGroups.get(point.seriesKey) || []; existing.push(point); seriesGroups.set(point.seriesKey, existing); });
    const processedSeries = Array.from(seriesGroups.entries()).map(([seriesKey, points], seriesIndex) => {
      const seriesColor = colors[seriesIndex % colors.length] || DEFAULT_COLORS[0];
      const processedPoints = points.map(point => ({
        ...
        radius: typeof size === 'number' || !size ? (typeof size === 'number' ? size : DEFAULT_POINT_SIZE) : scaleValue(point.sizeValue, [sizeMin, sizeMax], sizeRange),
        color: seriesColor, formattedX: formatValue(point.xValue), formattedY: formatValue(point.yValue), formattedSize: size && typeof size !== 'number' ? formatValue(point.sizeValue) : null,
      }));
      let trendLine = null;
      if (showTrendLine && processedPoints.length >= 2) {
        const regression = calculateLinearRegression(processedPoints.map(p => ({ x: p.xValue!, y: p.yValue! })));
        const lineX1 = effectiveXDomain[0], lineX2 = effectiveXDomain[1];
        const lineY1 = regression.slope * lineX1 + regression.intercept, lineY2 = regression.slope * lineX2 + regression.intercept;
        trendLine = { ...regression, screenX1: scaleValue(lineX1, effectiveXDomain, [0, chartWidth]), screenY1: scaleValue(lineY1, effectiveYDomain, [chartHeight, 0]), screenX2: scaleValue(lineX2, effectiveXDomain, [0, chartWidth]), screenY2: scaleValue(lineY2, effectiveYDomain, [chartHeight, 0]) };
      }
      return { key: seriesKey, color: seriesColor, points: processedPoints, trendLine };
    });
  ...
  if (error) return <states.Error error={error} />;
  if (!data.length) return <states.Empty />;
  if (!containerWidth) return <div ref={containerRef} className={cn('relative w-full', className)} style={{ height }}><div className="flex items-center justify-center h-full text-sm text-muted-foreground">Loading...</div></div>;

  const getGridDasharray = () => gridStyle === 'solid' ? 'none' : gridStyle === 'dotted' ? '2 4' : '4 4';
  const hoveredData = (() => {
  ...
  return (
    <div ref={containerRef} className={cn('relative w-full', className)} style={{ height }}>
      <svg width="100%" height={height} className="overflow-visible">
        <g transform={\`translate(\${MARGIN.left}, \${MARGIN.top})\`}>
          {showGrid && (<>{yTicks.map((tick, i) => { const tickY = scaleValue(tick, effectiveYDomain, [chartHeight, 0]); return <line key={\`grid-h-\${i}\`} x1={0} y1={tickY} x2={chartWidth} y2={tickY} stroke="currentColor" opacity={0.1} strokeDasharray={getGridDasharray()} />; })}
          {xTicks.map((tick, i) => { const tickX = scaleValue(tick, effectiveXDomain, [0, chartWidth]); return <line key={\`grid-v-\${i}\`} x1={tickX} y1={0} x2={tickX} y2={chartHeight} stroke="currentColor" opacity={0.1} strokeDasharray={getGridDasharray()} />; })}</>)}
          <line x1={0} y1={0} x2={0} y2={chartHeight} stroke="currentColor" opacity={0.3} strokeWidth={1.5} />
          <line x1={0} y1={chartHeight} x2={chartWidth} y2={chartHeight} stroke="currentColor" opacity={0.3} strokeWidth={1.5} />
          {yTicks.map((tick, i) => { const tickY = scaleValue(tick, effectiveYDomain, [chartHeight, 0]); return <g key={\`y-tick-\${i}\`}><line x1={-4} y1={tickY} x2={0} y2={tickY} stroke="currentColor" opacity={0.3} /><text x={-8} y={tickY} textAnchor="end" dominantBaseline="middle" fontSize={11} className="fill-muted-foreground">{formatValue(tick)}</text></g>; })}
          {xTicks.map((tick, i) => { const tickX = scaleValue(tick, effectiveXDomain, [0, chartWidth]); return <g key={\`x-tick-\${i}\`}><line x1={tickX} y1={chartHeight} x2={tickX} y2={chartHeight + 4} stroke="currentColor" opacity={0.3} /><text x={tickX} y={chartHeight + 16} textAnchor="middle" fontSize={11} className="fill-muted-foreground">{formatValue(tick)}</text></g>; })}
          {showTrendLine && processedSeries.map((series, si) => series.trendLine && <motion.line key={\`trend-\${si}\`} x1={series.trendLine.screenX1} y1={series.trendLine.screenY1} x2={series.trendLine.screenX2} y2={series.trendLine.screenY2} stroke={trendLineColor || series.color} strokeWidth={2} strokeDasharray="6 4" opacity={0.6} {...(animation && { initial: { pathLength: 0, opacity: 0 }, animate: { pathLength: 1, opacity: 0.6 }, transition: { duration: 0.8, delay: 0.5 + si * 0.1, ease: [0.4, 0, 0.2, 1] } })} />)}
          {processedSeries.flatMap((series, si) => series.points.map((point, pi) => {
            const isHovered = hoveredPoint?.index === point.index && hoveredPoint?.series === series.key;
            return <motion.circle key={\`point-\${si}-\${pi}\`} cx={point.screenX} cy={point.screenY} r={isHovered ? point.radius * 1.2 : point.radius} fill={point.color} stroke="white" strokeWidth={2} className="cursor-pointer" style={{ transformOrigin: \`\${point.screenX}px \${point.screenY}px\` }}
              onMouseEnter={() => setHoveredPoint({ index: point.index, series: series.key })} onMouseLeave={() => setHoveredPoint(null)} onClick={() => onPointClick?.(point.data, point.index, series.key)}
              {...(animation && { initial: { scale: 0, opacity: 0 }, animate: { scale: 1, opacity: 1 }, transition: { duration: 0.4, delay: si * 0.1 + pi * 0.02, ease: [0.4, 0, 0.2, 1] } })} />;
          }))}
        </g>
      </svg>
      {hoveredData && (<motion.div initial={{ opacity: 0, y: 5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 5 }} className="absolute pointer-events-none z-50 bg-background border rounded-lg px-3 py-2 shadow-xl transform -translate-x-1/2" style={{ left: hoveredData.point.screenX + MARGIN.left, top: Math.max(10, hoveredData.point.screenY + MARGIN.top - 80) }}>
        {processedSeries.length > 1 && <div className="flex items-center gap-2 mb-1"><div className="w-2 h-2 rounded-full" style={{ backgroundColor: hoveredData.color }} /><span className="text-xs font-medium">{hoveredData.seriesKey}</span></div>}
        <div className="text-xs text-muted-foreground">X: <span className="font-medium text-foreground">{hoveredData.point.formattedX}</span></div>
        <div className="text-xs text-muted-foreground">Y: <span className="font-medium text-foreground">{hoveredData.point.formattedY}</span></div>
        {hoveredData.point.formattedSize && <div className="text-xs text-muted-foreground">Size: <span className="font-medium text-foreground">{hoveredData.point.formattedSize}</span></div>}
      </motion.div>)}
      {showLegend && processedSeries.length > 1 && <div className="flex flex-wrap gap-4 justify-center mt-2 px-4">{processedSeries.map((series) => <div key={series.key} className="flex items-center space-x-2"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: series.color }} /><span className="text-sm text-muted-foreground">{series.key}</span></div>)}</div>}
    </div>
  );
}
